...
fn group_for_id(id: usize, dist: &str) -> usize {
    match dist {
        "range" => (id / 200) % 100,
        _ => id % 100,
    }
...
fn expected_group_rows(total_rows: usize, dist: &str, group: usize) -> usize {
    (0..total_rows)
        .filter(|id| group_for_id(*id, dist) == group)
        .count()
}
...
fn new_record_batch(start_id: i64, num_rows: usize, dist: &str) -> RecordBatch {
    ...
    let grps: Vec<i64> = ids
        .iter()
        .map(|v| group_for_id(*v as usize, dist) as i64)
        .collect();
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if num_tasks == 0 {
        return Err("IL_BENCH_TASKS must be greater than 0".into());
    }
    if inline_limit == 0 {
        return Err("IL_BENCH_INLINE_LIMIT must be greater than 0".into());
    }
    if row_group_size == 0 {
        return Err("IL_BENCH_ROW_GROUP_SIZE must be greater than 0".into());
    }
    let batch_size = (total_rows / num_tasks / 10).max(1);
    println!(
    ...
    // Equality filter scan (grp == 42), exercising row-level predicate filtering
    // and row-group stats pruning.
    let expected_group_rows = expected_group_rows(total_rows, &dist, 42);
    for run in 0..rounds {
        let scan = TableScan::default()
            .with_partition(TableScanPartition::single_partition())
        ...
        assert_eq!(
            rows, expected_group_rows,
            "filter scan row mismatch: expected {expected_group_rows}, got {rows}"
        );
    }
