...
use indexlake_integration_tests::data::{
    prepare_simple_fixed_size_vector_table, prepare_simple_vector_table,
};
...
| id | vector             | distance           |
+----+--------------------+--------------------+
| 3  | [30.0, 30.0, 30.0] | 6.928203105926514  |
| 2  | [20.0, 20.0, 20.0] | 10.392304420471191 |
+----+--------------------+--------------------+
"#,
    );
...
#[case(async { catalog_postgres().await }, async { storage_s3().await }, DataFileFormat::ParquetV2)]
#[tokio::test(flavor = "multi_thread")]
async fn create_hnsw_index_fixed_size_list(
    #[future(awt)]
    #[case]
    catalog: Arc<dyn Catalog>,
    #[future(awt)]
