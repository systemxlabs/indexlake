...
pub(crate) async fn read_parquet_file_with_row_group_pruner(
    ...
    // hand, judged by DataFusion's pruning predicate (built once per scan and
    // shared across files). Intersect the row-group selection with the
    // validity (delete bitmap) selection; `prune_file_row_groups` abandons
    // pruning when the footer row total doesn't match the catalog record
    // count, so the two selections are always aligned on the same row grid
    // here.
    let row_selection = {
